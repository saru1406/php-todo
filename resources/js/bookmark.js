$(function ()
{
    ...
    $('.toggle_wish').on('click', function ()
    {
        ...
        bookmark_memo = $(this).attr("bookmark_memo");
        click_button = $(this);
        ...
        $.ajax({
            ...
            url: '/bookmarks',  //route.phpで指定したコントローラーのメソッドURLを指定
            type: 'POST',   //GETかPOSTメソットを選択
            data: { 'memo_id': memo_id, 'bookmark_memo': bookmark_memo, }, //コントローラーに送るに名称をつけてデータを指定
                })
            ...
            .done(function (data) //コントローラーからのリターンされた値をdataとして指定
            {
                ...
                {
                    //クリックしたタグのステータスを変更
                    click_button.attr("bookmark_memo", "1");
                    //クリックしたタグの子の要素を変更(表示されているハートの模様を書き換える)
                    click_button.children().attr("class", "fas fa-map-pin");
                }
                if ( data == 1 )
                {
                    //クリックしたタグのステータスを変更
                    click_button.attr("bookmark_memo", "0");
                    //クリックしたタグの子の要素を変更(表示されているハートの模様を書き換える)
                    click_button.children().attr("class", "fas fa-map-pin");
                }
            })
